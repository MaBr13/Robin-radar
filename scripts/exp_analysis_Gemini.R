...
track <- sqlQuery(Gdb, "select id,timestamp_start,classification_id,airspeed, extract(year from timestamp_start) as year,extract (month from timestamp_start) as month, 
                  extract (day from timestamp_start) as day,extract (hour from timestamp_start) as hour  from public.track 
                  where timestamp_start between date '2019-08-15' and date'2019-11-30'
...
flock <- subset(track, classification_id>=8 & classification_id<=9, select = id:l.day) 
small <- subset(track, classification_id>=14 & classification_id<=15, select = id:l.day)
medium <- subset(track, classification_id>=12 & classification_id<=13, select = id:l.day)
big <- subset(track, classification_id>=10 & classification_id<=11, select = id:l.day)
...
allbirdsm$bird <- factor(allbirdsm$class, levels=c(8,9,10,11,12,13,14,15),labels=c("Flock", "Flock","Big birds", "Big birds","Medium birds", "Medium birds","Small birds","Small birds"))
...
ggplot(allbirdsm, aes(Date,trackph)) +
  ...
  ggtitle("Number of tracks Gemini Autumn 2019") +
  theme_bw()+
  ...
  ylim(0,250000)+
scale_x_date(date_breaks="months", date_labels="%b", limits=c(as.Date("2019-08-15"), as.Date("2019-11-30")))
...
vertical <- sqlQuery(Gdb, "select id,timestamp_end as timestamp,classification_id,
                     ST_Z(ST_EndPoint(trajectory)) as alt
                     from public.track
                     WHERE classification_id between 4 and 15
                     and timestamp_start between date'2019-08-15' and date '2019-11-30'
                     and extract(hour from timestamp_start) in (19,20,21,22,23,0,1,2,3,4,5,6,7)
...
ggplot(vertical, aes(alt)) +
  ...
  ggtitle("Flight altitudes Gemini Autumn 2019") +
  coord_flip()+
  theme_bw()+
  ...
  xlab("Altitudes") + ylab("Number of tracks")+ ylim(0,1200000) +
...
ggplot(vertical, aes(alt)) +
  ...
  theme(axis.title.y = element_text(size=18), legend.text=element_text(size=14), 
        ...
        axis.text.y=element_text(size=14), axis.text.x=element_text(size=12), plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"),
        axis.title.x = element_text(size=18), plot.title = element_text(size = 18, face = "bold"),panel.grid = element_blank())+
  xlab("Altitudes") + ylab("Number of tracks")+  ylim(0,250000) +
