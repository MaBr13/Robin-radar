...
rr_vertical_query <- function(public, day=TRUE){
  ...
  if(day){
	...
	WITH y AS (
		WITH x AS (
  			WITH radar AS (
  			  SELECT *, timestamp start_time, lead(timestamp) over (order by timestamp) end_time FROM rr_user.radar WHERE radar_type = 'VERTICAL'
  				)
			...
			track.*, 
			public.rise_set_time(st_y(position),st_x(position),TRUE,timestamp_start) sunrise,
			public.rise_set_time(st_y(position),st_x(position),FALSE,timestamp_start) sunset
			FROM x, public.track 
			WHERE timestamp_start BETWEEN '2020-02-01 00:00:00' AND '2020-06-01 00:00:00' --AND avg_corrected_mass between 29.40 and 70.80 
			AND
			tracktype  in ('RaEl','RaAzEl') 
		)
	SELECT track_id,
	avg(st_z) altitude,
	...
	avg_corrected_mass
	FROM y JOIN rr_user.classification cc on (cc.id=y.classification_id) 
	GROUP BY track_id, radar_location, st_centroid, distance, classification, avg_corrected_mass,timestamp_start, timestamp_start::date, extract(hour from timestamp_start),
...
con <- dbConnect("PostgreSQL", 
                    dbname='rws01', # database name  
                    host='robin1.e-ecology.nl', 
                    user= "maja_bradaric",# username
                    password="55nebitno@") # password

# import the data from the vertical radar using the vertical radar database 
tt <- rr_vertical_query('public', # select the schema
                        day=FALSE) # only tracks during the night (1h after sunset and 1h before sunrise)
# filter tracks to keep only birds (removes mainly insects)
tt_filtered <- subset(tt, avg_corrected_mass >= 29.40 & avg_corrected_mass <= 70.80)

# generate the density figure. Remove 
...
rr_vertical_density(data=tt_filtered, 
                    res=50,
                    limits=c(1,200), # just an example to set the limits of the color scale (1 so that all cells with 0 tracks are grey)
                    vertical_range=c(0,4000),
                    horizontal_range=c(-2000,2000),
                    dir=paste0(dir,toupper(database)))
