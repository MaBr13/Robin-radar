...
CREATE OR REPLACE FUNCTION rr_user.force_range(
  v DOUBLE PRECISION,
...
CREATE OR REPLACE FUNCTION rr_user.rise_set_time(
...
L = rr_user.force_range(L, 360.0);

-- 5a. calculate the Sun's right ascension
    RA = (1/RADIANS(1)) * ATAN(0.91764 * TAN(RADIANS(L)));
    RA = rr_user.force_range( RA, 360 );  -- NOTE: RA adjusted into the range [0,360);
...
-- 7a. calculate the Sun's local hour angle
    ...
    UT = T - longitude_hour;
    UT = rr_user.force_range( UT, 24);  -- UTC time in decimal format (e.g. 23.23)

    -- 10. Return
    hr = rr_user.force_range(UT::INT, 24);
...
COMMENT ON FUNCTION rr_user.rise_set_time(double precision, double precision, boolean, timestamp with time zone, double precision)
    IS 'function to calculate sunrise and sunset. Example:
  SELECT rr_user.rise_set_time(39.399872, -8.224454, TRUE, NOW()) AS rise,
rr_user.rise_set_time(39.399872, -8.224454, FALSE, NOW()) AS set;';
