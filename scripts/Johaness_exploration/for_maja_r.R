...
rr_vertical_query <- function(schema, day=TRUE){
  ...
  if(day){
	...
	WITH y AS (
		WITH x AS (
			...
			track.*, 
			rr_user.rise_set_time(st_y(position),st_x(position),TRUE,timestamp_start) sunrise,
			rr_user.rise_set_time(st_y(position),st_x(position),FALSE,timestamp_start) sunset
			FROM x, ",schema,".track 
...
tt <- rr_vertical_query(schema='m201904', # select the schema
