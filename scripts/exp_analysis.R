...
extract (day from timestamp_start) as day,extract (hour from timestamp_start) as hour  from public.track 
                        where timestamp_start between date '2018-09-21' and date'2018-09-26'
...
track <- sqlQuery(Rdb, "select id,timestamp_start,classification_id,airspeed, extract(year from timestamp_start) as year,extract (month from timestamp_start) as month, 
extract (day from timestamp_start) as day,extract (hour from timestamp_start) as hour  from public.track 
                   where timestamp_start between date '2018-10-11' and date'2018-10-22'
                   and extract(hour from timestamp_start) in (19,20,21,22,23,0,1,2,3,4,5,6,7)
                   and classification_id>4
                   order by timestamp_start")

track <- sqlQuery(Rdb,"select*,extract(year from timestamp_start) as year,extract (month from timestamp_start) as month, 
extract (day from timestamp_start) as day,extract (hour from timestamp_start) as hour  from public.track
                  where timestamp_start between date '2018-09-16' and date'2018-10-16'
                  and classification_id>4
                  order by timestamp_start")
close(Rdb)
...
medium <- subset(track, classification_id==7, select = id:l.day)
big <- subset(track, classification_id==6, select = id:l.day)
...
allbirdsm$bird <- factor(allbirdsm$class, levels=c(5,6,7,8),labels=c("Flock", "Big birds", "Medium birds", "Small birds"))
...
ggplot(allbirdsm, aes(Date,trackph)) +
  ...
  ggtitle("Number of tracks September - October 2018") +
